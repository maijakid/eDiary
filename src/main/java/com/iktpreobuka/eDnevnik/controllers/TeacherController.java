...
//				}
	...
	public  ResponseEntity<?> addSubjectById (@PathVariable Long teacherId, @PathVariable Long subjectId) {
		TeacherEntity te = teacherRepository.findOne(teacherId);
		SubjectEntity se = subjectRepository.findOne(subjectId);
		try {		//provere dal je vec dodat i ostalo
			if ((te != null) && (se != null)) {
			...
			return new ResponseEntity<RESTError>(new RESTError(1, "NEKO not found"), HttpStatus.NOT_FOUND);
		} catch (Exception e) {
			return new ResponseEntity<RESTError>(new RESTError(2, "Exception occurred: " + e.getMessage()), HttpStatus.INTERNAL_SERVER_ERROR);
			}
	}
